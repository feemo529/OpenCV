...
//
//	Mat rotaion0, img_warp0;
//
//	double angle = 30;	//设置图像旋转的角度
//	Size dst_size(img.rows, img.cols);	//设置输出图像的尺寸
//	Point2f center(img.rows / 2.0, img.cols / 2.0);	//设置图像的旋转中心
//
//	rotaion0 = getRotationMatrix2D(center, angle, 1); //计算仿射变换矩阵
//
//	warpAffine(img, img_warp0, rotaion0, dst_size); //进行仿射变换
...
//	dst_points[2] = Point2f((float)(img.rows)*0.81, (float)(img.cols)*0.80);
//
